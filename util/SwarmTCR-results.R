...
setRefWeights <- function(refweights.dfs, epitopes.df, directory = "./reference-data/", append.directory = "../reference-data/", method = "mediod", threshold = NULL) {
  ...
  if (!is.null(threshold)) {
    ...
    # Plot and print refweights
    (plot <- plotRefWeights(all.weights, set.weights, directory))
    print(plot)
    ggsave(paste0(directory, "refWeights.png"), plot, width = 20, height = 10)
...
setScoreThresholds <- function(pr.dfs, scores.dfs, epitopes, directory = "./reference-data/", method = "precision") {
  ...
  for (epitope in epitopes) {
    ...
    if (method == "precision") {
      ...
      threshold <- getScoreFbeta(pr, scores, epitope)
    }
  ...
  thresholds <- thresholds %>%
    ...
    dplyr::slice_head(n=1) %>%
    ungroup()
  plotScoreThresholds(pr.dfs, scores.dfs, epitopes, thresholds, directory)
...
getScoreFbeta <- function(pr, scores, epitope, beta = 0.5) {
  optimal <- pr %>%
    mutate(Fbeta = (1 + beta^2) * Precision * Recall / (beta^2 * Precision + Recall),
      F1 = 2 * Precision * Recall / (Precision + Recall)) %>% 
