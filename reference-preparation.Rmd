...
# Here, we generate a total of 1,500 negative sequences per epitope in the positive dataset from a wider pool of reference data, excluding cross-reactive data
n.negative <- 1500
exclude.source <- "chan"
...
The makeClassifierReference function creates a reference file for SwarmTCR based on the positive reference data and the target epitopes. This is used in SwarmTCRClassify, after SwarmTCR pretraining, but is most easily generated at this step. 
...
To run SwarmTCR pretraining using the newly formatted data, copy each data directory to your SwarmTCR directory and run the following command:
